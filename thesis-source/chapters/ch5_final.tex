...
This chapter describes how that client is implemented on Pepper.
The client runs as a NAOqi service, connects QiChat grammar rules to Python methods, captures images, collects robot metadata, calls the server from Chapter~\ref{chap:server_side_rewrite}, speaks answers, refreshes dynamic dialogue concepts, and displays the current scene memory on the tablet.
The emphasis here is implementation: the concrete service boundary, turn execution, grammar integration, metadata payloads, and development adapters that realize the high-level design.
...
This registration is what makes QiChat service calls possible.
For example, the quick-look rule in Listing~\ref{lst:qichat-client-sample} calls the service method that starts a visual turn.
The service itself is intentionally kept as a boundary object.
It exposes methods for quick look, scan, chat, object-focused chat, cached answers, memory display, memory reset, configuration reload, and status inspection, but it delegates slow work to specialized components.
...
This adapter structure is practical on Pepper because NAOqi services differ in availability and failure behavior.
For example, the real robot provides \texttt{ALVideoDevice} and \texttt{ALTabletService}, while a virtual robot in Choregraphe does not provide the same camera and tablet behavior.
The implementation therefore includes \texttt{FakeCameraAdapter} and \texttt{FakeTabletAdapter}.
The fake camera reads images from a local directory and returns them through the same interface as the real camera adapter.
The fake tablet starts a small local HTTP server and exposes the tablet HTML application in a desktop browser.
Both are selected through \texttt{client\_config.json}, so the same turn manager can be tested locally and then run on the physical robot without changing the high-level workflow.
...
This gives the robot a deterministic interaction layer.
The VLM and LLM components do not replace QiChat; they are called only after the grammar has selected an action such as quick look, scan, object question, memory display, or cached answer.
This separation is important for a robot because the user-facing entry points must remain predictable even when the downstream server uses generative models.
...
The full scene graph and object instances remain on the server, where they can be used for grounding and memory reasoning.

This means the grammar changes with the perceived scene.
...
The original QiChat or tablet call can therefore return quickly instead of blocking the NAOqi service while the robot captures images or waits for the server.
...
Pepper can therefore speak a short caption as soon as the server returns it, while the server continues updating detections, scene graph relations, memory, and dynamic concepts for later questions.
...
This matters for implementation because the client must handle limited resolution, robot camera exposure, motion, and network transmission rather than assuming a clean image dataset.
...
The practical distinction is therefore clear.
...
The metadata is useful because Pepper is not a passive camera.
...
The server parses this payload into typed metadata objects and can use it during detection, memory update, scene graph generation, and social attribute construction.
...
The guarded turn execution catches these errors and speaks localized fallback messages.
This is preferable to exposing raw HTTP failures through the robot, because users need to know that the robot failed gracefully rather than silently stopping.
...
The tablet code is written in an older ES5-style global namespace style because the Pepper tablet environment is limited compared with a modern browser.
...
These helpers are useful during testing because Pepper deployment cycles are slow.
