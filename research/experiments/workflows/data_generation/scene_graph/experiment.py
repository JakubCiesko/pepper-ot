...
def run_detection(
    ...
    for i, (image_paths, image_batch) in enumerate(
        ...
        if (
            ...
            results = [detection_service.detect(image_batch[0])]
        else:
            results = detection_service.detect_batch(image_batch)
...
async def consolidate_vocabulary(
    ...
    output = {
        "predicates": pred_resp.parsed.predicates,
        "attributes": attr_resp.parsed.attributes,
    }
