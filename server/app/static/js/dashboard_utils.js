...
function displayPepperSentence(text) {
    ...
    div.className =
        "bg-slate-950 border border-slate-800 p-3 rounded shadow-sm text-slate-300";
...
function initTabGroup(groupName) {
    const buttons = Array.from(document.querySelectorAll(`[data-tab-group=\"${groupName}\"][data-tab-target]`));
...
function initPageNavigation() {
    const navItems = Array.from(document.querySelectorAll(".nav-item[data-page-target]"));
    ...
    navItems.forEach(btn => {
        btn.addEventListener("click", () => activate(btn.getAttribute("data-page-target")));
    });
